...
module.exports = {
    ...
    run: async function (client, message, args) {
        ...
        if (!permissions.has("CONNECT")) return sendError("Please gimme connecr permission to join!", message.channel);
        if (!permissions.has("SPEAK")) return sendError("Please gimme speak permission to play!", message.channel);

        var searchString = args.join(" ");
        if (!searchString) return sendError("Hey sus, please gimme song name to play!", message.channel);
        ...
        if (serverQueue) {
            ...
            let thing = new MessageEmbed()
                .setColor("#F7E5BC")
                .addField("Added to queue", song.title, true)
        ...
        const play = async (song) => {
            ...
            let thing = new MessageEmbed()
                .setColor("#F7E5BC")
                .addField("Now playing", song.title, true)
